...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        self.final_conv = nn.Conv2d(input_size, 11, kernel_size=1)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_down_layers):
            ...
            downsampling_indices.append(indices)
        
        downsampling_indices = downsampling_indices[::-1] # reverse indices list

        # Upsampling path
        for i in range(self.num_up_layers):
            x = self.layers_unpooling[i](x, downsampling_indices[i])
