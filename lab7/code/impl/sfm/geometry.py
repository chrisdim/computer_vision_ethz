...
def EstimateEssentialMatrix(K, im1, im2, matches):
  ...
  normalized_kps1 = (np.linalg.inv(K)@MakeHomogeneous(im1.kps, 1).transpose()).transpose()
  normalized_kps2 = (np.linalg.inv(K)@MakeHomogeneous(im2.kps, 1).transpose()).transpose()
  ...
  sigma = np.array([1,1,0]) #  The first two singular values need to be equal, the third one zero.
  E = (U*sigma)@ V
...
def TriangulatePoints(K, im1, im2, matches):
  ...
  points3D_homogenous =  MakeHomogeneous(points3D, 1).transpose()
  points3D_im1 = (P1@ points3D_homogenous).transpose() # to filter points for the first camera such that Z>0
  points3D_im2 = (P1@ points3D_homogenous).transpose() # to filter points for the second camera such that Z>0

  MASK = np.logical_and(points3D_im1[:,-1]>0, points3D_im2[:,-1]>0) 
...
def EstimateImagePose(points2D, points3D, K):  
  ...
  normalized_points2D = (np.linalg.inv(K) @ MakeHomogeneous(points2D, 1).transpose()).transpose()
...
def TriangulateImage(K, image_name, images, registered_images, matches):
  ...
  for reg_name in registered_images:
        offset = len(points3D)
        points3D_new, im1_corrs, im2_corrs = TriangulatePoints(K, image, images[reg_name], GetPairMatches(image_name, reg_name, matches))
        points3D = np.append(points3D, points3D_new, axis=0)
        corrs[image_name] = np.array([im1_corrs, offset+np.arange(points3D_new.shape[0])])
        corrs[reg_name] = np.array([im2_corrs, offset+np.arange(points3D_new.shape[0])])
